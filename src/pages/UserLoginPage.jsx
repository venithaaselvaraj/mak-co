...
export default function UserLoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err.message?.includes('invalid') ? 'Invalid email or password' : 'Failed to sign in. Please try again.');
    }
    setLoading(false);
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#FBF6E9] relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 w-64 h-64 bg-amber-900/5 rounded-full blur-2xl -translate-x-1/2 -translate-y-1/2"></div>
      ...
      <div className="w-full max-w-md mx-4 relative z-10">
        ...
        <div className="bg-white/40 backdrop-blur-md border border-amber-900/10 rounded-[2rem] p-10 shadow-2xl">
          ...
          <p className="text-center text-[#5D4037]/60 mt-8 text-[10px] uppercase tracking-widest font-bold">
            New to the Circle?{' '}
            <Link to="/signup" className="text-[#800000] hover:text-[#A52A2A] transition-colors border-b border-[#800000]/20 pb-0.5">
              Sign Up
            </Link>
          </p>
        </div>
